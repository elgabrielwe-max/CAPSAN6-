...
ssomaRouter.get('/dashboard',async(req,res)=>{
  const unitIds=req.user.role==='MASTER'?null:req.user.units.map(x=>Number(x.id));
  const k=(await pool.query(`SELECT COUNT(*) FILTER(WHERE p.plan_date=CURRENT_DATE+1)::int plans_tomorrow,COUNT(*) FILTER(WHERE p.status='COMPLETADO')::int completed_plans FROM ssoma_work_plans p WHERE ($1::int[] IS NULL OR p.business_unit_id=ANY($1::int[]))`,[unitIds])).rows[0];
  const evidence=Number((await pool.query(`SELECT COUNT(*)::int total FROM ssoma_evidence e WHERE ($1::int[] IS NULL OR e.business_unit_id=ANY($1::int[])) AND e.evidence_date>=CURRENT_DATE-30`,[unitIds])).rows[0].total);
  const pending=(await pool.query(`SELECT bu.name,COUNT(r.id) FILTER(WHERE r.status<>'LEVANTADO')::int total
    FROM business_units bu LEFT JOIN racs r ON r.business_unit_id=bu.id
    WHERE bu.active=TRUE AND ($1::int[] IS NULL OR bu.id=ANY($1::int[]))
    GROUP BY bu.id,bu.name ORDER BY total DESC,bu.name`,[unitIds])).rows;
  res.json({kpis:{...k,evidenceLast30:evidence},pending});
});
