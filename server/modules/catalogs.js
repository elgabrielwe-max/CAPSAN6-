...
import { fetchRacCauseCatalog } from '../services/racCatalog.js';
...
catalogsRouter.get('/',async(req,res)=>{
  ...
  const racCauseCategories=await fetchRacCauseCatalog(pool);
  res.json({units,areas,users:people,roles:['MASTER','SSOMA','SUPERVISOR'],racStatuses:['PENDIENTE','EN PROCESO','PENDIENTE DE VALIDACION','DEVUELTO PARA CORRECCION','LEVANTADO'],riskLevels:['ALTO','MEDIO','BAJO'],racCauseCategories});
});
