...
dashboardRouter.get('/',async(req,res)=>{
  ...
  const workerScope=req.user.role==='MASTER'?'TRUE':`w.business_unit_id=ANY($1::int[])`;
  const workerParams=req.user.role==='MASTER'?[]:[req.user.units.map(x=>Number(x.id))];
  const workers=Number((await pool.query(`SELECT COUNT(*)::int total FROM workers w WHERE active=TRUE AND ${workerScope}`,workerParams)).rows[0].total||0);
  const trainings=Number((await pool.query(`SELECT COUNT(*)::int total FROM trainings WHERE enabled=TRUE`)).rows[0].total||0);
  const incidents=Number((await pool.query(`SELECT COUNT(*)::int total FROM flash_reports WHERE followup_status<>'CERRADO'`)).rows[0].total||0);
