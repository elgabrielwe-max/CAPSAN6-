...
const aliases = {
  ...
  cause:['TIPO DE DESVIACION','DESVIACION','PROBLEMA','CAUSA','SUBTIPO'],
  ...
  lifted:['LEVANTAMIENTO','SE LEVANTO','ESTADO'], progress:['LEVANTAMIENTO','AVANCE','PORCENTAJE']
};
...
export function analyzeRacWorkbook(buffer, fileName='archivo.xlsx', options={}) {
  ...
  rows.forEach(row=>{const n=sourceNumber(field(row,'code'));if(n)sourceCounts.set(n,(sourceCounts.get(n)||0)+1);});
  ...
  for(let i=0;i<rows.length;i++){
    ...
    const ai=classifyRacLocal(`${field(row,'cause')} ${description}`);
    ...
    const hash=crypto.createHash('sha1').update([unitName,date,source,occurrence,reporter,description].join('|')).digest('hex').slice(0,12).toUpperCase();
    records.push({
      sourceRow, sourceReportNumber:source, internalCode:`${options.unitCode||'RAC'}-${date.replaceAll('-','')}-${String(source).padStart(4,'0')}${occurrence>1?`-${String(occurrence).padStart(2,'0')}`:''}-${hash.slice(0,4)}`,
      ...
      riskLevel:risk(field(row,'risk')), reportType, causeCategory:ai.causeCategory, causeSubtype:cause.toUpperCase(), deviationType:cause.toUpperCase(),
  ...
  return { sheetName:chosen.name, headerRow:chosen.headerRow+1, totalRows:rows.length, validRows:records.length, rejectedRows:errors.length, records, errors, warnings, periods:periodList, dominantPeriod:periodList[0]?.period||null, repeatedNumbers:repeated };
}
