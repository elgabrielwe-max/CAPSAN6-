...
test('4.0.26 queda versionado y renueva recursos web',()=>{
  assert.match(read('server/index.js'),/CAPSAN6 4\.0\.31 ejecutándose/);
  assert.match(read('server/app.js'),/4\.0\.26-LISTADO-DIRECCIONADO-EDICION-RACS-CATALOGO-TIPOS-DE-CAUSA/);
  assert.match(read('package.json'),/"version": "4\.0\.31"/);
  assert.match(read('public/index.html'),/v=4031/);
  assert.match(read('public/js/app.js'),/racs\.js\?v=4031/);
});
