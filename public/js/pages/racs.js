...
async function directedTab(){
  ...
  const box=$('#racOps');
  box.innerHTML=`<section class="panel"><div class="change-list-head"><div><h3>Listado direccionado</h3><div class="panel-sub">Exclusivo para Máster y SSOMA. Corrige clasificación, define el área responsable y registra por qué debe levantar la observación.</div></div><span class="tag high">NO VISIBLE PARA SUPERVISORES</span></div><div class="filter-grid"><div class="field"><label>Unidad</label><select id="directUnit">${unitOptions()}</select></div><div class="field"><label>Situación</label><select id="directStatus"><option value="NOT_DIRECTED">Sin direccionar</option><option value="DIRECTED">Direccionados</option><option value="ALL">Todos</option></select></div><div class="field"><label>Área direccionada</label><select id="directArea">${areaOptions()}</select></div><div class="field"><label>Buscar</label><input id="directSearch" placeholder="RAC, causa, lugar, área o motivo"></div><div class="field"><label>&nbsp;</label><button class="btn primary" id="loadDirected">Actualizar listado</button></div></div><div id="directedList"></div></section>`;
  async function load(){
    ...
    try{
      ...
      target.innerHTML=rows.length?table(['RAC','Unidad','Área reportada','Área direccionada','Motivo','Riesgo','Tipo / causa','Estado',''],rows.map(r=>`<tr><td><b>${escapeHtml(r.report_code)}</b><br><small>${escapeHtml(r.source_report_number||'')}</small></td><td>${escapeHtml(r.business_unit||'')}</td><td><small>Reportante: ${escapeHtml(r.reporting_area||'—')}</small><br><b>${escapeHtml(r.reported_area||'—')}</b></td><td>${r.directed_area?`<span class="tag done">${escapeHtml(r.directed_area)}</span><br><small>${escapeHtml(r.directed_by_name||'')} · ${formatDateTime(r.directed_at)}</small>`:'<span class="tag pending">SIN DIRECCIONAR</span>'}</td><td>${escapeHtml(r.direction_reason||'—')}</td><td><span class="tag ${r.risk_level==='ALTO'?'high':r.risk_level==='MEDIO'?'medium':'low'}">${escapeHtml(r.risk_level)}</span></td><td><small>${escapeHtml(r.report_type||'')}</small><br><b>${escapeHtml(r.cause_category||'')}</b><br>${escapeHtml(r.cause_subtype||r.deviation_type||'')}</td><td>${statusTag(r.status)}</td><td><button class="btn small amber" data-direct="${r.id}">${r.directed_area_id?'Editar / redireccionar':'Direccionar y corregir'}</button></td></tr>`)):'<div class="empty">No existen RACS para los filtros seleccionados.</div>';
