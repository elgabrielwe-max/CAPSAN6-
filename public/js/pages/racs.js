...
async function newTab(){
  ...
  form.onsubmit=async event=>{event.preventDefault();const submittedForm=event.currentTarget;if(!categorySelect.value||!subtypeSelect.value)return toast('Selecciona el tipo de causa y la subcausa','error');try{const result=await api('/api/racs',{method:'POST',body:formData(submittedForm)});toast(`RAC ${result.report_code} registrado`);submittedForm.reset();submittedForm.elements.reportDate.value=new Date().toISOString().slice(0,10);$('#workerText').value='';$('#aiResult').innerHTML='';refreshCategories();}catch(error){toast(error.message,'error')}};
}
