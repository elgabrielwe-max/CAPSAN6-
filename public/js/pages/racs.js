...
const changeTitle=action=>({CREATE_RAC:'RAC registrado',ASSIGN_RAC:'Supervisor asignado',UPDATE_RAC_STATUS:'Cambio de seguimiento'}[action]||'Actualización');
const changeDescription=change=>{
  const details=change.details||{};
  if(change.action==='UPDATE_RAC_STATUS')return `${escapeHtml(details.from||'SIN ESTADO')} → <b>${escapeHtml(details.to||'ACTUALIZADO')}</b>${details.comment?` · ${escapeHtml(details.comment)}`:''}`;
  ...
  async function load(){
    ...
    try{
      ...
      target.innerHTML=rows.length?`<div class="rac-change-list">${rows.map(rac=>{
        ...
        }).join('')}</div>`:'<div class="empty-evidence">Todavía no se ha cargado evidencia para este RAC.</div>';
        ...
        return `<article class="rac-change-card"><header><div><div class="rac-change-code">${escapeHtml(rac.report_code)}</div><div class="rac-change-source">${escapeHtml(rac.source_report_number||'SIN NÚMERO DE ORIGEN')} · Último cambio ${formatDateTime(rac.last_change_at)}</div></div><div class="rac-change-tags"><span class="tag ${rac.risk_level==='ALTO'?'high':rac.risk_level==='MEDIO'?'medium':'low'}">${escapeHtml(rac.risk_level)}</span>${statusTag(rac.status)}</div></header><div class="rac-change-grid"><div><small>Unidad</small><b>${escapeHtml(rac.business_unit||'')}</b></div><div><small>Fecha del RAC</small><b>${String(rac.report_date||'').slice(0,10)}</b></div><div><small>Tipo</small><b>${escapeHtml(rac.report_type||'')}</b></div><div><small>Supervisor</small><b>${escapeHtml(rac.supervisor_name||'SIN ASIGNAR')}</b></div><div><small>Área reportante</small><b>${escapeHtml(rac.reporting_area||'')}</b></div><div><small>Área reportada</small><b>${escapeHtml(rac.reported_area||'')}</b></div><div><small>Lugar</small><b>${escapeHtml(rac.location||'—')}</b></div><div><small>Avance</small><b>${Number(rac.progress_percent||0)}%</b></div></div><div class="rac-change-detail"><div><small>Causa / subcausa</small><p><b>${escapeHtml(rac.cause_category||'')}</b> · ${escapeHtml(rac.cause_subtype||rac.deviation_type||'')}</p></div><div><small>Descripción del RAC</small><p>${escapeHtml(rac.description||'')}</p></div>${rac.corrective_action?`<div><small>Acción correctiva</small><p>${escapeHtml(rac.corrective_action)}</p></div>`:''}</div><div class="rac-change-columns"><section><h4>Evidencias (${evidences.length})</h4>${evidenceHtml}</section><section><h4>Historial de cambios (${changes.length})</h4><div class="change-timeline">${timeline}</div></section></div></article>`;
