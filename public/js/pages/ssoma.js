...
export async function resourcesPage(root){
  ...
  <section class="panel"><div class="page-head compact"><div><h3>Control RACS por unidad</h3><p>Plazos institucionales: ALTO 0–48 horas · MEDIO 1–3 días · BAJO 1–4 días.</p></div><button class="btn amber" id="racControlExcel">Excel Control RACS por unidad</button></div><div id="racControlKpis"></div><div id="racControlTable"><div class="page-loading"><span class="spinner"></span>Cargando control…</div></div></section>
  ...
  async function loadControl(){
    try{
      ...
      const total=sum(rows,'total'),lifted=sum(rows,'lifted'),overdue=sum(rows,'overdue'),pendingValidation=sum(rows,'pending_validation'),withoutEvidence=sum(rows,'lifted_without_evidence');
      $('#racControlKpis').innerHTML=`<div class="kpi-grid">${kpi('RACS',total,'Periodo filtrado','navy')}${kpi('Levantados',lifted,total?`${Math.round(lifted*100/total)}% de cierre`:'Sin RACS','green')}${kpi('Vencidos',overdue,'Fuera del plazo','red')}${kpi('Pendientes de validación',pendingValidation,'Con evidencia por revisar','amber')}${kpi('Levantados sin evidencia',withoutEvidence,'Regularización documental','coral')}</div>`;
      $('#racControlTable').innerHTML=table(['Unidad','RACS','Alto','Medio','Bajo','Levantados','Lev. sin evidencia','Pend. validación','Vencidos','Alto vencido','% cierre'],rows.map(row=>`<tr><td><b>${escapeHtml(row.unit)}</b><br><small>${Number(row.workers||0)} trabajadores</small></td><td>${row.total}</td><td>${row.high}</td><td>${row.medium}</td><td>${row.low}</td><td>${row.lifted}</td><td>${row.lifted_without_evidence}</td><td>${row.pending_validation}</td><td>${row.overdue}</td><td>${row.high_overdue}</td><td>${row.total?Math.round(Number(row.lifted)*100/Number(row.total)):0}%</td></tr>`));
